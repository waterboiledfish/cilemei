...
**答：** 为了更好的体验，采用「流式输出」：后端在拿到 AI 的完整回复后，按段落或换行拆成小块，每隔约 120ms 向响应体写一块，前端用 `fetch` 的 `response.body.getReader()` 读流，每读到一块就追加到页面，并自动滚动到底部。这样用户能尽快看到首屏内容，减少等待焦虑。在 Vercel 免费版 10 秒函数限制下，文字建议约 9 秒超时；拍照识别为降低超时概率，视觉接口设为 9.5 秒超时，且前端上传前会对图片压缩（最长边 1024px、JPEG 质量 0.82），后端 prompt 精简，超时后返回明确提示，避免页面一直卡住。

---

### Q11：拍照识别食物时，模型返回 JSON 不规范怎么办？

**答：** 部分模型有时会返回带 markdown 代码块、多余逗号或重复字段的「类 JSON」文本。后端做了容错：（1）先剥掉可能的 \`\`\`json ... \`\`\` 包裹再 `JSON.parse`；（2）解析成功后统一成规范字段（支持中英 key：foods/食物、advice/建议、cautions/提醒等），`cautions` 只保留一条非空字符串，避免重复刷屏；（3）若 `JSON.parse` 失败，用正则从文本里提取 foods、nutrition_notes、advice、cautions（支持多种 key 名），再格式化为易读文本展示，绝不把整段原始 JSON 直接给用户看。
...
**答：** Vercel 免费版 Serverless 函数有约 10 秒的执行上限，若 DashScope 响应超过该时间，函数会被平台终止，前端就会表现为卡住或收不到完整响应。我们做了多重缓解：（1）文字建议约 9 秒超时；（2）拍照识别：视觉接口 9.5 秒超时（可通过环境变量 `AI_VISION_TIMEOUT_MS` 微调），前端在上传前用 canvas 将图片压缩为最长边 1024px、JPEG 质量 0.82，后端 prompt 精简以减少 token，从而缩短模型响应时间。超时后返回「AI 视觉识别超时，请稍后重试或换一张图」等明确提示。若仍常超时，可换更清晰、简单的食物图或升级 Vercel 计划。
...
**答：** 亮点：（1）从注册、健康档案到文字+图像两种 AI 建议形成完整闭环；（2）流式输出、图片缩放与拖拽、建议历史（自动入库、可回看）；（3）针对识别超时做了优化：前端上传前压缩图片、视觉接口 9.5s 超时、prompt 精简；（4）AI 返回格式容错：剥离代码块、统一中英字段、cautions 只展示一条、解析失败时正则抽取不展示原始 JSON；（5）技术选型兼顾开发效率与免费部署（Vercel + Supabase + DashScope），密码与鉴权、环境变量有基本安全考虑。不足：（1）免费版 Vercel 10 秒限制下，个别场景仍可能超时；（2）历史暂无分页或删除单条；（3）若误提交过 `.env` 需轮换密钥。后续可针对这些点做迭代。

---

### Q20：你在这个项目里负责哪些部分？遇到的最大难点是什么？

**答：**（可根据实际分工调整）我负责整体架构设计、后端 API 与数据库设计、Vercel/Supabase 的接入，以及前端与后端的联调。遇到的主要难点包括：（1）Vercel 上 Express 不能直接作为根入口，需要改成 Serverless 入口 + 共用 Express 路由；（2）Supabase 直连在 Vercel 环境下 IPv4 兼容问题，需改用 pooler URI；（3）模型有时返回非标准 JSON，需要兼容解析与格式化输出（剥离代码块、统一中英 key、cautions 不重复）；（4）拍照识别在 10 秒限制下易超时，通过前端上传前压缩图片、视觉接口 9.5s 超时、精简 prompt 等方式缓解。通过查文档、看报错日志和逐步缩小范围排查解决了这些问题。
